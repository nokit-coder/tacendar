...
void Action::print_debug() { printf("check action %lu(%i)\n", id, status); }
...
void Action::load_from_db(sqlite3_stmt *stmt) {
	for (int i = 0; i < sqlite3_column_count(stmt); i++) {
		...
		if (name == "type") 
			type = static_cast<ActionType>(sqlite3_column_int(stmt, i));
		if (name == "status") 
			status = static_cast<Status>(sqlite3_column_int(stmt, i));
	}
}
...
void ActionCommand::print_debug() {
  printf("check action %lu(%i) : \"%s\" : %i : \"%s\"\n", id, status,
         command.c_str(), exit_code, output.c_str());
}

void ActionCommand::load_from_db(sqlite3_stmt *stmt) {
  Action::load_from_db(stmt);

	// прикольно отфармотировал
  // if ( idx_cmd > 0  ) {  command   =  reinterpret_cast<const char *>(sqlite3_column_text( stmt, idx_cmd  ));  }
  // if ( idx_exit > 0 ) {  exit_code =                                 sqlite3_column_int ( stmt, idx_exit ) ;  }
  // if ( idx_out > 0  ) {  output    =  reinterpret_cast<const char *>(sqlite3_column_text( stmt, idx_out  ));  }

	for (int i = 0; i < sqlite3_column_count(stmt); i++) {
		std::string name = sqlite3_column_name(stmt, i);
		if (name == "command")
			command = reinterpret_cast<const char *>(sqlite3_column_text( stmt, i ));
		if (name == "exit_code") 
			exit_code = sqlite3_column_int64(stmt, i);
		if (name == "output") 
			output = reinterpret_cast<const char *>(sqlite3_column_text( stmt, i ));
	}
}
